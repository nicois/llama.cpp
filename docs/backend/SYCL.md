...
| GGML_SYCL_MEMTRACE | 0 (default, disabled), 1, 2 | Enable more detailed memory allocation diagnostics, including current and peak allocations, as well allocations from other sources, around 50 lines per model load. (2) is more verbose, logging around 900 specific allocations and deallocations. Either level requires `-lv 4`, since backend logging is only enabled at that verbosity. |
