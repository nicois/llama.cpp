...
class Server:
    ...
    def snapshot(self):
        ...
        if total is None or free is None:
            raise SystemExit(
                "error: llamacpp:vram_{total,free}_bytes absent.\n"
                "  Upstream llama-server does not export VRAM gauges; they come from a\n"
                "  local /metrics extension. This script cannot work without them, since\n"
                "  measuring real device memory is its entire purpose. Either run a build\n"
                "  that exports them, or read VRAM from `llama-server --list-devices` on\n"
                "  the host. Also check the server was started with --metrics.")
        if total == free:
            raise SystemExit(
