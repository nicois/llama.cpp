...
def test_linger_disabled_with_empty_deferred_queue():
    ...
    assert "__TEST_TAG_SLOT_LINGER_HIT__" not in drained
    assert "__TEST_TAG_SLOT_LINGER_EXPIRED__" not in drained
...
def test_linger_hit_serves_in_window_arrival_first():
    ...
    def old_request():
        ...
        old_result = server.make_request("POST", "/completion", data={
    ...
    def new_request():
        ...
        blocking_done.wait()
        time.sleep(0.05)  # Small delay to ensure linger has started
        new_result = server.make_request("POST", "/completion", data={
    ...
    drained = log.drain()
    assert "__TEST_TAG_SLOT_LINGER_HIT__" in drained, \
...
def test_linger_expired_uses_fifo():
    ...
    def late_request():
        ...
        late_result = server.make_request("POST", "/completion", data={
            "prompt": prompt_c, "cache_prompt": True
        })
    ...
    drained = log.drain()
    assert "__TEST_TAG_SLOT_LINGER_EXPIRED__" in drained, \
...
def test_linger_does_not_drop_concurrent_slot_releases():
    """Two slots releasing inside one linger window must each still promote a deferred task.

    A single global linger record would let the second release overwrite the first, so only
    one deferred task is promoted per window and the run needs two windows instead of one.
    """
    global server
    server.n_slots = 2
    ...
    results = parallel_function_calls([
        (server.make_request, ("POST", "/completion", {"prompt": p, "cache_prompt": True}))
        for p in prompts
    ])
    elapsed = time.monotonic() - start
...
def test_no_linger_when_a_deferred_task_is_bound_to_the_slot():
    ...
    def occupier():
        results["a"] = server.make_request("POST", "/completion", data={
            "prompt": prompt_a, "cache_prompt": True
        })
    ...
    time.sleep(0.1)  # the unpinned request must take slot 0 first, else nothing is deferred

    # arrives while slot 0 is busy, so it is deferred while pinned to slot 0
    results["b"] = server.make_request("POST", "/completion", data={
        "prompt": prompt_b, "id_slot": 0, "cache_prompt": True
    })
