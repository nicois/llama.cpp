...
#include "fattn.hpp"   // ggml_sycl_fattn_extra / ggml_sycl_fattn_get_extra
...
void launch_fattn(
    ...
    // Prefer the space reserved on `dst` by get_alloc_size; fall back to the legacy
    // grow-only side buffer when reservation is disabled (GGML_SYCL_FA_RESERVE=0).
    ...
    if (need_f16_K && K->type != GGML_TYPE_F16) {
        ...
        sycl::half * K_f16_ptr = extra.K ? (sycl::half *) extra.K
    ...
    if (need_f16_V && V->type != GGML_TYPE_F16) {
        if (V_is_K_view) {
            ...
            sycl::half * V_f16_ptr = extra.V ? (sycl::half *) extra.V
